...
from maru.applications import programme_domain_reference_views as views
...
from tests.unit import test_application_programme_domain_references as contracts
...
def owner_queries_without_database(world, monkeypatch):
    monkeypatch.setattr(views, "_domain_options", world.options)
...
def incoming(world, payload=None, *, edit=True, query="", csrf=False, revision_id=None):
    ...
    path = root + f"work/answer/{world.answer.question.question_id}/domain/"
    ...
    return views.programme_domain_reference(
        request,
        world.organization,
        world.edition,
...
def confirmation(world, token, **changes):
    return (
        evidence(
            ...
            token=token,
            target="",
            reason="Reference this call entry",
            confirm="on",
        )
        | changes
    )
...
def test_confirmation_dispatches_only_original_target_and_source_versions(world):
    token = selected(world)
...
def test_clear_requires_two_deliberate_posts_and_writes_none(world):
    response = incoming(world, evidence(world, mode="clear", target=""))
    assert response.status_code == 200
...
def test_invalid_confirmation_preserves_original_proof_and_selected_label(
    ...
    assert "Culture" in soup.get_text()
    world.writers["append_programme_proposal_answer"].assert_not_called()
...
def test_stale_fresh_writer_retains_reason_confirmation_and_original_proof(world):
    ...
    assert "Reference this call entry" in soup.get_text()
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"target": [str(UUID(int=40)), str(UUID(int=41))]},
        {"reason": "a" * 6001},
        {"phase": "unknown"},
    ],
)
...
def test_current_and_frozen_viewers_render_label_without_raw_target_id(world, frozen):
    response = incoming(
        world,
        edit=False,
    ...
    assert "Labels and guidance belong to this immutable call" in text
...
def test_label_loss_during_real_render_withholds_all_original_bytes(world, monkeypatch):
    ...
    assert b"Culture" not in response.content


@pytest.mark.parametrize(("error", "status"), [(Denied, 404), (DatabaseError, 503)])
def test_dependency_and_authority_failures_never_disclose_target(world, error, status):
...
def test_registered_routes_remain_dormant(world):
    path = (
        f"/my/applications/programme/{world.organization}/{world.edition}/"
        f"{world.context.summary.proposal_id}/references/domain/"
        f"{world.answer.question.question_id}/"
    )
    assert (
        resolve(path, urlconf="maru.applications.programme_proposal_urls").url_name
        == "my-programme-domain-reference"
    )
    with pytest.raises(Resolver404):
        resolve(path)


def test_real_chooser_is_labelled_and_never_a_raw_identifier_text_editor(world):
    soup = BeautifulSoup(incoming(world).content, "html.parser")
    choices = soup.select_one('select[name="target"]')
    assert choices is not None
    assert "Culture (culture)" in choices.get_text()
    assert soup.select_one('input[name="target"]') is None


def test_stale_prepare_keeps_original_choice_without_loading_current_catalog(world):
    response = incoming(world, evidence(world, expected_call_version="999"))
    assert response.status_code == 409
    soup = BeautifulSoup(response.content, "html.parser")
    retained = soup.select_one('select[name="target"] option[selected]')
    assert retained["value"] == str(world.option.target_id)
    assert "Original choice retained" in retained.get_text()
    assert "Culture" not in soup.get_text()
    world.options.assert_not_called()
    world.writers["append_programme_proposal_answer"].assert_not_called()


def test_catalog_change_during_render_withholds_prepared_bytes(world, monkeypatch):
    original = views._html.__globals__["render_to_string"]

    def render(*args, **kwargs):
        content = original(*args, **kwargs)
        world.options.return_value = (replace(world.option, label="Changed"),)
        return content
