...
def test_application_templates_are_same_shell_and_free_of_mojibake() -> None:
    ...
    for template_path in template_root.glob("*.html"):
        ...
        if template_path.name in {
            ...
            assert ('{% include "applications/' + template_path.name + '" %}') in parent
        ...
        elif template_path.name == "_programme_file_metadata.html":
            for parent_name in ("programme_file.html", "programme_review_file.html"):
                parent = (template_root / parent_name).read_text(encoding="utf-8")
                assert (
            ...
            assert "<h1" not in source
            assert "<main" not in source
        elif template_path.name == "programme_decision_source.html":
            parent = (template_root / "programme_decisions.html").read_text(
                encoding="utf-8"
            )
            for context in ("row", "message"):
                assert (
                    '{% include "applications/programme_decision_source.html" '
                    f"with source={context}.source only %}}"
                ) in parent
            assert '{% extends "admin/base_site.html" %}' in parent
            assert "{% extends" not in source
            assert "<main" not in source
