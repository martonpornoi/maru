...
@pytest.mark.parametrize(
    ("capability", "field"),
    [
        (auth.VIEW_HOST_SELF, "own_host_schedule"),
        (auth.VIEW_WORK_SELF, "own_work_schedule"),
    ],
)
def test_personal_schedule_policy_uses_exact_self_not_edition_planner(
    monkeypatch, capability, field
):
