"""Independently admitted exact-review-answer domain viewers with anonymous omission."""
...
from .programme_domain_references import (
...
@transaction.atomic
def get_programme_review_domain_reference(
    *,
    request: ProgrammeReviewReadRequest,
    case_id: UUID,
    question_key: str,
    ...
    """Resolve an exact admitted nonanonymous answer, never a caller target identifier.
    ...
    -------
    ProgrammeDomainReferenceView
        Immutable call label/guidance and source facts for final render comparison.
    ...
    answer = selected[0]
    if (
        answer.get("type") != "domain_reference"
        or answer.get("reference_kind") not in DOMAIN_REFERENCE_KINDS
    ):
        raise Denied
    call_id = source[-1]
    ...
    if _source(request, case_id, assignment_id, authorizer) != source:
        raise Denied
    _audit(request, "domain_reference", case_id, authorizer)
    return result
