"""Declare exact conversion authority without granting or activating it."""
...
CONVERSION_CAPABILITY = "applications.convert_programme_acceptance"
ORGANIZATION_CAPABILITIES = _previous.ORGANIZATION_CAPABILITIES
EDITION_CAPABILITIES = _previous.EDITION_CAPABILITIES
DEPARTMENT_CAPABILITIES = (*_previous.DEPARTMENT_CAPABILITIES, CONVERSION_CAPABILITY)
...
def refuse_used_conversion_capability_downgrade(apps: Any, schema_editor: Any) -> None:
    """Retain conversion vocabulary after durable grant or role evidence."""
    ...
    if grant.objects.filter(capability_code=CONVERSION_CAPABILITY).exists() or (
        bundle.objects.filter(capability_codes__contains=[CONVERSION_CAPABILITY]).exists()
    ):
        raise RuntimeError(
            "Cannot remove Programme conversion vocabulary after durable authority "
            "evidence exists; retain compatible code and fix forward."
        )


class Migration(migrations.Migration):
    """Install exact Department scope with an unused-only reverse."""
    ...
    operations: ClassVar[list[object]] = [
        ...
        migrations.RunPython(
            ...
            reverse_code=refuse_used_conversion_capability_downgrade,
        ),
    ]
