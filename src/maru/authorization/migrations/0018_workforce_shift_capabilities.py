"""Add exact-edition Shift view and management capability scope rules."""
...
SHIFT_CAPABILITIES = ("workforce.view_shifts", "workforce.manage_shifts")


def _capability_sql(*, include_shifts: bool) -> str:
    edition_codes = EDITION_CAPABILITIES + (
        SHIFT_CAPABILITIES if include_shifts else ()
    )
...
def refuse_used_shift_capability_downgrade(
    apps: Any,
    schema_editor: Any,
) -> None:
    """Refuse removing Shift capabilities once authority references them."""
    ...
    used = (
        capability_grant.objects.filter(capability_code__in=SHIFT_CAPABILITIES).exists()
        or role_bundle.objects.filter(
            capability_codes__overlap=list(SHIFT_CAPABILITIES)
        ).exists()
    )
    if used:
        raise RuntimeError(
            "Cannot remove Workforce Shift authority after durable grants or role "
            "evidence exists; keep compatible code and fix forward."
        )


class Migration(migrations.Migration):
    """Install PostgreSQL Shift capability scope and downgrade fencing."""
    ...
    operations: ClassVar[list[object]] = [
        migrations.RunSQL(
            _capability_sql(include_shifts=True),
            reverse_sql=_capability_sql(include_shifts=False),
        ),
        migrations.RunPython(
            migrations.RunPython.noop,
            reverse_code=refuse_used_shift_capability_downgrade,
        ),
    ]
