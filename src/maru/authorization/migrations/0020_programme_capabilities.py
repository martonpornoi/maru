"""Add exact-edition capability scope rules for dormant Programme contracts."""
...
def _capability_sql(*, include_programme: bool) -> str:
    edition_codes = EDITION_CAPABILITIES + (
        PROGRAMME_CAPABILITIES if include_programme else ()
    )
...
def refuse_used_programme_capability_downgrade(
    apps: Any,
    schema_editor: Any,
) -> None:
    """Refuse contraction once durable Programme authority evidence exists."""
    ...
    used = (
        capability_grant.objects.filter(
            capability_code__in=PROGRAMME_CAPABILITIES
        ).exists()
        or role_bundle.objects.filter(
            capability_codes__overlap=list(PROGRAMME_CAPABILITIES)
        ).exists()
    )
    if used:
        raise RuntimeError(
            "Cannot remove Programme authority after durable grants or role evidence "
            "exists; keep compatible code and fix forward."
        )


class Migration(migrations.Migration):
    """Install PostgreSQL Programme capability scope and downgrade fencing."""
    ...
    operations: ClassVar[list[object]] = [
        migrations.RunSQL(
            _capability_sql(include_programme=True),
            reverse_sql=_capability_sql(include_programme=False),
        ),
        migrations.RunPython(
            migrations.RunPython.noop,
            reverse_code=refuse_used_programme_capability_downgrade,
        ),
    ]
