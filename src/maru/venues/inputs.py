"""Pure bounded normalization for venue commands."""
...
from .models import MAX_VENUE_REASON_LENGTH

if TYPE_CHECKING:
    from collections.abc import Mapping

MAX_SOURCE_CHANNEL_LENGTH = 32
MAX_VENUE_SLUG_LENGTH = 80
...
def normalized_text(
    ...
    *,
    ...
    if not isinstance(value, str):
        _field_error(field, "Enter text for this field.", "venue_text_invalid")
    if any(unicodedata.category(character) == "Cc" for character in value):
        _field_error(
            field,
            "Control characters are not allowed.",
            "venue_control_character",
        )
    normalized = unicodedata.normalize("NFC", value).strip()
    if collapse:
        normalized = " ".join(normalized.split())
    if required and not normalized:
        _field_error(field, "This field is required.", "venue_value_required")
    if len(normalized) > maximum:
        _field_error(
            field,
            f"Ensure this value has at most {maximum} characters.",
            "venue_value_too_long",
        )
    return normalized
...
def normalized_reason(value: str) -> str:
    ...
    return normalized_text(
        value,
        field="reason",
        maximum=MAX_VENUE_REASON_LENGTH,
        required=True,
        collapse=True,
    )
...
def normalized_slug(value: str, *, fallback: str = "") -> str:
    ...
    result = slugify(raw)[:MAX_VENUE_SLUG_LENGTH].strip("-")
    if not result:
        _field_error("slug", "Enter a stable slug.", "venue_slug_invalid")
    return result
