...
| Programme intake/review | Dormant Applications import/intake and Programme foundations / runtime inactive | ADRs 0081–0084 and PRG-001–011 define the Applications-to-Programme boundary, proposal collaboration, preview-first migration, ownership continuity, later host relationships, privacy, and progressive profile. Applications owns dormant call/submission facets, collaborative revisions, staged import, non-disclosing preview, protected apply/exact-self claim, permanent source binding, and explicit disposal. Issue #64 adds shared Department-retirement locking, reasoned reassignment, and dormant exact-ID recovery. Programme owns guarded item/readiness layers and reviewed public copy. Both current profile fingerprints remain unchanged and dormant relations are runtime `SELECT`-only. No route, API, UI, connector, review/decision, accepted-item adapter, host relationship, Scheduling continuation, or setup behavior is activated. |
...
ADRs 0081 through 0084 contract the Applications-to-Programme seam: Applications
...
Department-retirement continuation now has dormant reassignment, disposal,
recovery, and shared-lock enforcement. The profile, review/decision path, and
accepted adapter remain non-executable.
...
has a mounted journey. Issue #64 supplies Department-retirement coordination.
The full milestone exit still requires Programme-specific review/decisions and the
...
- [x] Applications-owned dormant Programme calls, exact collaborative proposal
  revisions, included-collaborator acknowledgements, legacy-target denial, and database
  dormancy; no Programme item or host relationship follows.
