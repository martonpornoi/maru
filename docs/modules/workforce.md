...
Status: Initial position, opportunity, agreement, and authority-onboarding slice  
...
has a controller. The **First convention setup** admin wizard and its
`bootstrap_convention` operator fallback are therefore one shared one-shot,
trust-on-first-use exception. They require:
...
- the logged-in superuser's current password in the web wizard; and
...
a superuser can follow **First convention setup** in bootstrap administration.
The logged-in superuser is the controller, so there are no controller-email or
database arguments to mistype. The wizard delegates to exactly the same
transaction, validation, audit, and one-shot checks as the command.

The command remains useful for recovery rehearsal and automation. In
PowerShell, set the database in a separate statement and invoke the virtual
...
The current bootstrap-admin form identifies the second controller and checks
their live authority. A production approval inbox with a separate approver
session and step-up remains future work; selecting an identity in the local
rehearsal must not be represented as that future UX.
