...
organizer-visible draft. The dormant Programme schema is installed, but the
demo command intentionally creates no Programme rows or purpose relationships.
Dealer-table, accommodation, and case records remain future modules.
Registration is a real vertical with a local/test-only payment adapter. See
